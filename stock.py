...
from datetime import datetime, timedelta # argparse removed as input() is used
...
try:
    ...
    GNews = None
    print("Warning: 'gnews' library not found. GNews functionalities will be skipped. Run 'pip install gnews'.")

# --- IMPORTANT: INSTALL NECESSARY LIBRARIES ---
# If you encounter ModuleNotFoundError, run these commands in your terminal:
# pip install yfinance textblob newsapi-python requests gnews
#
...
# export GNEWS_API_KEY="your_actual_gnews_api_key_here"
...
newsapi_client = None
if NewsApiClient:  # Only attempt to initialize if the NewsApiClient class was successfully imported
    if NEWS_API_KEY:
        try:
            newsapi_client = NewsApiClient(api_key=NEWS_API_KEY)
            print("NewsAPI client initialized.")
        except Exception as e:
            print(f"Error initializing NewsAPI client even with API key: {e}")
            newsapi_client = None # Ensure client is None on error
    else:
        print("NEWS_API_KEY environment variable not set. NewsAPI client not initialized, functionalities will be skipped.")
# If NewsApiClient is None (library not found), the top-level import warning has already informed the user.

# Initialize GNews client
gnews_client = None
if GNews: # Only attempt if GNews class was imported
    if GNEWS_API_KEY:
        try:
            gnews_client = GNews() # Initialize without api_key argument
            # The GNews library typically picks up the GNEWS_API_KEY from the environment
            print("GNews client initialized.")
        except Exception as e:
            print(f"Error initializing GNews client: {e}")
            gnews_client = None
    else:
        print("GNEWS_API_KEY environment variable not set. GNews functionalities will be skipped.")
...
def calculate_technical_indicators(historical_data: pd.DataFrame) -> dict:
    ...
    if len(df) < 200: # Max window size for SMA_200
...
def analyze_stock(historical_data: pd.DataFrame, news_sentiment: float = None) -> tuple:
    ...
    if technical_indicators['SMA_5'] is not None and technical_indicators['SMA_20'] is not None:
        if technical_indicators['SMA_5'] > technical_indicators['SMA_20']:
            buy_signals += 1
            reasons.append("Positive short-term trend (5-day avg price > 20-day avg)")
        elif technical_indicators['SMA_5'] < technical_indicators['SMA_20']:
            sell_signals += 1
            reasons.append("Negative short-term trend (5-day avg price < 20-day avg)")
        else:
            hold_signals += 1
            reasons.append("Neutral short-term trend (5-day avg price similar to 20-day avg)")

    # RSI
    if technical_indicators['RSI'] is not None:
        if technical_indicators['RSI'] < RSI_OVERSOLD_THRESHOLD:
            buy_signals += 1
            reasons.append(f"Stock may be oversold/undervalued (RSI: {technical_indicators['RSI']:.2f})")
        elif technical_indicators['RSI'] > RSI_OVERBOUGHT_THRESHOLD:
            sell_signals += 1
            reasons.append(f"Stock may be overbought/overvalued (RSI: {technical_indicators['RSI']:.2f})")
        else:
            hold_signals += 1
            reasons.append(f"Market momentum is neutral (RSI: {technical_indicators['RSI']:.2f})")
    ...
    if technical_indicators['MACD'] is not None and technical_indicators['MACD_Signal'] is not None:
        if technical_indicators['MACD'] > technical_indicators['MACD_Signal']:
            buy_signals += 1
            reasons.append("Positive momentum indicated by MACD")
        elif technical_indicators['MACD'] < technical_indicators['MACD_Signal']:
            sell_signals += 1
            reasons.append("Negative momentum indicated by MACD")
        else:
            hold_signals += 1
            reasons.append("Neutral momentum indicated by MACD")

    # News Sentiment
    if news_sentiment is not None:
        if news_sentiment > POSITIVE_SENTIMENT_THRESHOLD:
            buy_signals += 1
            reasons.append(f"Recent news is generally positive (Sentiment: {news_sentiment:.2f})")
        elif news_sentiment < NEGATIVE_SENTIMENT_THRESHOLD:
            sell_signals += 1
            reasons.append(f"Recent news is generally negative (Sentiment: {news_sentiment:.2f})")
        else:
            hold_signals += 1
            reasons.append(f"Recent news sentiment is neutral (Sentiment: {news_sentiment:.2f})")
    ...
    if buy_signals > sell_signals and buy_signals >= hold_signals:
        ...
        return "Buy", confidence, "Key buy indicators: " + "; ".join(reasons)
    elif sell_signals > buy_signals and sell_signals >= hold_signals:
        confidence = min(100, int(sell_signals / total_signals * 100))
        return "Sell", confidence, "Key sell indicators: " + "; ".join(reasons)
    else:
        confidence = min(100, int(hold_signals / total_signals * 100))
        return "Hold", confidence, "Observations: " + "; ".join(reasons)
...
def enhanced_analysis(stock_symbol: str, historical_data: pd.DataFrame, technical_indicators: dict,
    ...
    if company_fundamentals: # Check if company_fundamentals is not None
...
def analyze_sentiment(text: str) -> float:
    ...
    if not isinstance(text, str) or not text.strip(): # Check for empty or whitespace-only strings
        return 0.0 # Return neutral sentiment for invalid input
...
def fetch_news_sentiment_from_newsapi(ticker_symbol: str) -> tuple[float | None, list[str]]:
    ...
    try:
        ...
        # Modify the query for Indian stocks to be more specific
        query = ticker_symbol
        if ticker_symbol.upper().endswith(".NS"):
            # For Indian stocks, be more explicit in the query to help NewsAPI
            # You might want to extract the base ticker, e.g., "RELIANCE" from "RELIANCE.NS"
            base_ticker = ticker_symbol.upper().replace(".NS", "")
            query = f'"{base_ticker}" India OR "{ticker_symbol}" stock OR "{base_ticker}" NSE' # Added more terms and quotes for exact matches

        query_params = {
            'q': query,
            'language': 'en',
            'sort_by': 'relevancy',  # Sort by relevancy to get the most relevant articles
            'from_param': from_date,
            'to': to_date,
            'page_size': 20  # Max 20 articles per request for free tier
        }
        articles_response = newsapi_client.get_everything(**query_params)
...
def fetch_news_sentiment_from_gnews(ticker_symbol: str) -> tuple[float | None, list[str]]:
    """
    Fetches news for a given Indian stock ticker symbol from GNews
    and calculates the average sentiment.
    ...
    Returns:
        tuple: (Average sentiment: float | None, List of news headlines: list)
    """
    if not gnews_client:
        print("GNews client not initialized. Cannot fetch news from GNews.")
        return None, []

    # Only fetch from GNews if it's an Indian stock
    if not ticker_symbol.upper().endswith(".NS"):
        print(f"Skipping GNews for non-Indian ticker: {ticker_symbol}")
        return None, []

    base_ticker = ticker_symbol.upper().replace(".NS", "")
    # Construct a query. GNews might work better with company name.
    # You might need to map base_ticker to a more common company name if just the ticker isn't enough.
    query = f'{base_ticker} stock news site:moneycontrol.com OR site:economictimes.indiatimes.com'
    # Alternative query: query = f'{base_ticker} stock news' and then set gnews_client.country = 'IN'

    try:
        gnews_client.country = 'IN' # Set country to India
        gnews_client.language = 'en' # Set language to English
        gnews_client.period = '7d' # News from the last 7 days
        gnews_client.max_results = 10 # Max 10 articles
        
        # For GNews, the query is set when calling get_news
        ...
        if articles:
            ...
            if sentiments:
                avg_sentiment = np.mean(sentiments)
                news_headlines = [article.get('title', 'No Headline') for article in articles]
                print(f"Fetched {len(news_headlines)} articles from GNews for {ticker_symbol} (query: '{query}').")
                return avg_sentiment, news_headlines
        print(f"No recent news found for {ticker_symbol} from GNews (query: '{query}').")
        return None, []
    except Exception as e:
        print(f"An unexpected error occurred while fetching GNews for {ticker_symbol}: {e}")
        return None, []
...
def get_stock_data(ticker_symbol: str) -> tuple[pd.DataFrame | None, float | None, dict | None, str | None]:
    ...
    """
    if yf is None:
        return None, None, None, "Yfinance library not available. Stock data functionalities skipped."
    ...
    try:
        ...
        if not company_fundamentals or (company_fundamentals.get('regularMarketPrice') is None and company_fundamentals.get('longName') is None and company_fundamentals.get('marketCap') is None):
        ...
        # Ensure company_fundamentals is populated even if fetched after history (though fetched before now)
        if not company_fundamentals: # Should not happen if the first check passed, but as a safeguard
             company_fundamentals = stock.info # Re-fetch if somehow empty
...
if __name__ == "__main__":
    ...
    ticker_input = input("Enter the stock ticker symbol to analyze (e.g., AAPL, GOOG): ")
    if not ticker_input.strip():
        print("No ticker symbol entered. Exiting.")
        exit()
    TICKER = ticker_input.strip().upper() # Convert to uppercase and remove leading/trailing whitespace
    ...
    if error:
        print(f"\nError: {error}")
        # If there was an error but some fundamentals were fetched (e.g., historical data failed later)
        if company_fundamentals and (company_fundamentals.get('longName') or company_fundamentals.get('regularMarketPrice')):
            print("\n--- Partial Fundamental Data (if available) ---")
            print(f"Company: {company_fundamentals.get('longName', TICKER)}")
            print(f"Market Price (from info): {company_fundamentals.get('regularMarketPrice', 'N/A')}")
            print(f"P/E Ratio: {company_fundamentals.get('trailingPE', 'N/A')}")
            print(f"Market Cap: {company_fundamentals.get('marketCap', 'N/A')}")
        print("\nExiting due to data fetching issues.")
    elif historical_data is None or historical_data.empty: # Should catch cases where historical data is specifically missing
        # This 'error' variable here would be the specific message from get_stock_data
        # if historical_data was empty but fundamentals were present.
        if error: # If get_stock_data returned a specific message for this case
            print(f"\nWarning: {error}")
        else: # Generic message if error was None but historical_data is still empty
            print(f"\nCould not retrieve sufficient historical data for {TICKER} to perform full analysis.")
        
        if company_fundamentals and current_price is not None: # If get_stock_data returned fundamentals and a price from info
            print(f"\n--- Limited Data for {TICKER} (No Historical Chart Data) ---")
            print(f"Current Price (from fundamentals): ${current_price:.2f}")
            print(f"Company: {company_fundamentals.get('longName', TICKER)}")
            print(f"P/E Ratio: {company_fundamentals.get('trailingPE', 'N/A')}")
        print("\nExiting due to missing historical data for full analysis.")
    else:
        print(f"\n--- Data for {TICKER} ---")
        ...
        gnews_sentiment, gnews_titles = fetch_news_sentiment_from_gnews(TICKER)
        ...
        # De-duplicate news titles to avoid processing/displaying the same news multiple times
        # This is a simple de-duplication based on exact title match.
        ...
        if overall_news_sentiment is not None:
            ...
            # The list is already de-duplicated above.
            # Display up to 10 unique titles.
        ...
        # Truncate sample news for cleaner console display if it's too long
        _display_sample_news_cli = sample_news_content_event.strip()
        if len(_display_sample_news_cli) > 200: # Adjusted truncation for CLI
            _display_sample_news_cli = _display_sample_news_cli[:200] + "..."
        print(f"Sample News: '{_display_sample_news_cli}'")
        ...
        print(f"\n\n--- KEY ANALYSIS SUMMARY FOR {TICKER} ---")
        print(f"Final Recommendation: {enhanced_recommendation}")
        print(f"Confidence Level: {enhanced_confidence}%")
        print(f"Primary Reasons: {enhanced_reason}")
        if alerts: # Display critical alerts from enhanced analysis again for emphasis
            print("Important Alerts to Consider:")
            for alert in alerts:
                print(f"  - {alert}")
